...
def gen_script(fw, src_dir, prj_name, kernel_name, iob, bp, lp1, lp2):
    ...
    fw.write('set_directive_resource -core RAM_1P "{}" x1\n'.format(kernel_name))
    fw.write('set_directive_resource -core RAM_1P "{}" x2\n'.format(kernel_name))
    fw.write('set_directive_resource -core RAM_1P "{}" y1\n'.format(kernel_name))
    fw.write('set_directive_resource -core RAM_1P "{}" y2\n'.format(kernel_name))
    fw.write('set_directive_interface -mode ap_fifo "{}" x1_out\n'.format(kernel_name))
    fw.write('set_directive_interface -mode ap_fifo "{}" x2_out\n'.format(kernel_name))

    # buffer partition
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" buff_A\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_x1\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_x2\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_y1\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_y2\n'.format(iob, kernel_name))
    ...
    fw.write('set_directive_pipeline "{}/lpwr"\n'.format(kernel_name))
    fw.write('set_directive_unroll -factor {} "{}/lpwr"\n'.format(iob, kernel_name))
