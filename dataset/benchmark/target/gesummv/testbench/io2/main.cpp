...
extern "C" void gesummv(DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE A_0[N*N/2], DATA_TYPE A_1[N*N/2], 
	DATA_TYPE B_0[N*N/2], DATA_TYPE B_1[N*N/2], DATA_TYPE x[N], DATA_TYPE y_out[N]);
...
int main(int argc, char* argv[])
{
    ...
    for(inv = 0; inv < INV_NUM; inv++){
		...
		for(i = 0; i < N; i++){
		    for(j = 0; j < N; j++){
			    if(fscanf(fptr2, "%f", &read_din) > 0){
					...
					if(j%2 == 0) B_0[j_B_0++] = read_din;
					else B_1[j_B_1++] = read_din;
				}
		...
		//==============================
		gesummv(alpha, beta, A_0, A_1, B_0, B_1, x, y_out);
