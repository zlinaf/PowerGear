...
#############################################################
def gen_script(fw, prj_dir, src_dir, ip_dir, prj_name, iob):
    src_name = 'io_p{}'.format(iob)
    fw.write('create_project prj {}/{} -part xczu9eg-ffvb1156-2-e\n'.format(prj_dir, prj_name))    
...
#############################################################
def traverse_directive(fw, prj_dir, src_dir, ip_dir, prj_list, script_num = 1):
    ...
    for prj_name in prj_list:
        if(not prj_name == '') and  (not os.path.exists('{}/{}/prj.runs/impl_exe/wrapper.bit'.format(prj_dir, prj_name))):
            ...
            gen_script(fw[prj_cnt % script_num], prj_dir, src_dir, ip_dir, prj_name, iob)
            prj_cnt = prj_cnt + 1
            frun.write('{}\n'.format(prj_name))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hls_dir', help = 'directory of HLS project as input', action = 'store')
    parser.add_argument('--prj_dir', help = "directory to generate vivado project", action = 'store')
    parser.add_argument('--src_dir', help = "directory of RTL wrapper source", action = 'store')
    ...
    prj_cnt, terminate_cnt, finished_cnt = traverse_directive(fw, prj_dir, src_dir, ip_dir, prj_list, script_num)
    total_prj_cnt = len(prj_list)
    print('Total prj num to run = {}'.format(total_prj_cnt))
    print('Prj num to run = {}'.format(prj_cnt))
    print('Prj run finished = {}'.format(finished_cnt))
