...
def gen_script(fw, src_dir, prj_name, kernel_name, iob, bp, lp1, lp2):
    ...
    fw.write('set_directive_resource -core RAM_1P "{}" r\n'.format(kernel_name))
    fw.write('set_directive_resource -core RAM_1P "{}" p\n'.format(kernel_name))
    fw.write('set_directive_interface -mode ap_fifo "{}" s_out\n'.format(kernel_name))
    fw.write('set_directive_interface -mode ap_fifo "{}" q_out\n'.format(kernel_name))

    # buffer partition
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" buff_A\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_p\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_r\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_s_out\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_q_out\n'.format(iob, kernel_name))
