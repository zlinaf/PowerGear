...
from torch_geometric.nn import ASAPooling, LEConv, global_mean_pool, global_add_pool, global_max_pool, JumpingKnowledge
from hec_gnn.conv.Conv import HECConv,HECConv_no_relation
...
class HECConvNet(nn.Module):
    def __init__(self, in_channels, hidden_channels, num_layers,dim, 
        overall_dim = 0, use_overall: bool = False, batch_norm: bool = False,drop_out = 0.5,pool_aggr = "add",overall_dim_large = 128,relations = 4,aggregate = "add",simple_JK = "last",gnn_relation = True):
        super(HECConvNet, self).__init__()
        self.num_layers = num_layers
        self.convs = nn.ModuleList()
        for i in range(num_layers):
            in_channels = in_channels if i == 0 else hidden_channels
            if gnn_relation == True:
                self.convs.append(HECConv(in_channels, hidden_channels,dim,num_relation=relations,aggr=aggregate))
            else:
                self.convs.append(HECConv_no_relation(in_channels, hidden_channels,dim,num_relation=relations,aggr=aggregate))
        if use_overall:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gnn_relation', help = "have mutirelation layer", action = 'store', default = 1)
    ...
    parser.add_argument('--fold_index', help = "which fold is val", action = 'store', default = 4)
    ...
    gnn_relation = True if int(args.gnn_relation) > 0 else False
    ...
    if gnn_relation == True:
        ...
        gnn_mode = "HECConv_no_relation"
    print("gnn_mode:",gnn_mode)
    ...
    run_dir = '{a}/HECCov_relation<{h}>_{b}_{c}_{d}_{e}_{f}_overall<{i}>_gnn_realtion<{q}>_edge_feature<{j}>_nodes_feature<{k}>_onevone<{g}>_train_set<{l}>_aggregate<{m}>_pooling<{n}>_loss<{o}>_JK<{p}>'.format(a = model_dir, b = num_conv_layers, c = hidden_channels, d = batch_size, e = lr,f = drop_out,h = relations,i = overall_dim_large,j = edge_feature,k = node_feature,g =onevone, l = train_dataset,m = aggr_type , n = pool_type,o = loss_func,p = simple_JK,q = gnn_relation)
    if use_overall:
