...
def gen_script(fw, src_dir, prj_name, kernel_name, iob, bp, lp1, lp2):
    ...
    fw.write('set_directive_resource -core RAM_1P "{}" B\n'.format(kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" B\n'.format(iob, kernel_name))
    fw.write('set_directive_resource -core RAM_1P "{}" C\n'.format(kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" C\n'.format(iob, kernel_name))
    fw.write('set_directive_interface -mode ap_fifo "{}" D_out\n'.format(kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" D_out\n'.format(iob, kernel_name))

    # buffer partition
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" buff_A0\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" buff_A1\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" buff_B0\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" buff_B1\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" buff_C\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" buff_D_out\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" tmp1\n'.format(iob, kernel_name))
    fw.write('set_directive_array_partition -type cyclic -factor {} -dim 2 "{}" tmp2\n'.format(iob, kernel_name))

    if bp == 'd1d2':
        fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_A0\n'.format(iob, kernel_name))
        fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_A1\n'.format(iob, kernel_name))
        fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_B0\n'.format(iob, kernel_name))
        fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_B1\n'.format(iob, kernel_name))
        fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_C\n'.format(iob, kernel_name))
        fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" buff_D_out\n'.format(iob, kernel_name))
        fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" tmp1\n'.format(iob, kernel_name))
        fw.write('set_directive_array_partition -type cyclic -factor {} -dim 1 "{}" tmp2\n'.format(iob, kernel_name))

    # buff reading loop pipeline/unroll
    fw.write('set_directive_pipeline "{}/lprd_2"\n'.format(kernel_name))
    fw.write('set_directive_unroll -factor {} "{}/lprd_2"\n'.format(iob, kernel_name))
    fw.write('set_directive_pipeline "{}/lpwr_2"\n'.format(kernel_name))
    fw.write('set_directive_unroll -factor {} "{}/lpwr_2"\n'.format(iob, kernel_name))
    ...
    if lp1[0] == 'p':
        fw.write('set_directive_pipeline "{}/lp5"\n'.format(kernel_name))
        fw.write('set_directive_unroll -factor {} "{}/lp5"\n'.format(lp1[1], kernel_name))
    elif lp1[2] == 'p':
        fw.write('set_directive_pipeline "{}/lp6"\n'.format(kernel_name))
        fw.write('set_directive_unroll -factor {} "{}/lp6"\n'.format(lp1[3], kernel_name))
    else:
        fw.write('set_directive_unroll -factor {} "{}/lp6"\n'.format(lp1[3], kernel_name))

    if lp2[0] == 'p':
        fw.write('set_directive_pipeline "{}/lp7"\n'.format(kernel_name))
        fw.write('set_directive_unroll -factor {} "{}/lp7"\n'.format(lp2[1], kernel_name))
    elif lp2[2] == 'p':
        fw.write('set_directive_pipeline "{}/lp8"\n'.format(kernel_name))
        fw.write('set_directive_unroll -factor {} "{}/lp8"\n'.format(lp2[3], kernel_name))
    else:
        fw.write('set_directive_unroll -factor {} "{}/lp8"\n'.format(lp2[3], kernel_name))
