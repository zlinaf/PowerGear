...
from torch.nn import Parameter, Linear, BatchNorm1d,LSTM ,ReLU,Sequential
from torch_sparse import SparseTensor, matmul, masked_select_nnz
from torch_geometric.typing import PairTensor, Adj, OptTensor, Size, OptPairTensor
from torch_sparse import SparseTensor, matmul
from torch_geometric.data import Data
from torch_geometric.nn.conv import MessagePassing
from torch_geometric.nn import global_mean_pool, global_add_pool, global_max_pool, JumpingKnowledge,SAGEConv,GATConv,GCNConv,GINConv,GINEConv,GraphConv
...
class GraphConvNet(nn.Module):
    def __init__(self, in_channels, hidden_channels, num_layers,dim, 
        overall_dim = 0, use_overall: bool = False, batch_norm: bool = False,drop_out = 0.5,pool_aggr = "add",overall_dim_large = 128,relations = 4,aggregate = "add",simple_JK = "last",gnn_type = "GCN"):
        super(GraphConvNet, self).__init__()
        self.num_layers = num_layers
        self.gnn_type = gnn_type
        self.convs = nn.ModuleList()
        if self.gnn_type == "GINE":
            self.egde_dim_same_in_channels = Linear(dim,in_channels)
            self.egde_dim_same_hidden_channels = Linear(dim,hidden_channels)
        elif self.gnn_type == "GraphConv":
            self.egde_dim_to_one = Linear(dim,1)
        for i in range(num_layers):
            in_channels = in_channels if i == 0 else hidden_channels
            if gnn_type == "GCN":
                self.convs.append(GCNConv(in_channels, hidden_channels,aggr=aggregate))
            elif gnn_type == "GIN":
                self.convs.append(GINConv(in_channels, hidden_channels,aggr=aggregate))
            elif gnn_type == "GINE":
                self.convs.append(GINEConv(Sequential(Linear(in_channels, hidden_channels),ReLU(),Linear(hidden_channels, hidden_channels))))
            elif gnn_type == "GraphConv":
                self.convs.append(GraphConv(in_channels,hidden_channels,aggr=aggregate))
            elif gnn_type == "GraphSage":
                self.convs.append(SAGEConv(in_channels, hidden_channels,aggr=aggregate)) 
    ...
    def forward(self, data):
        ...
        h_list = [x]
        if self.gnn_type == "GINE":
            edge_attr_in_channels = self.egde_dim_same_in_channels(edge_attr)
            edge_attr_hidden_channels = self.egde_dim_same_hidden_channels(edge_attr) 
        elif self.gnn_type == "GraphConv":
            edge_attr_change_one = self.egde_dim_to_one(edge_attr)
        for i, conv in enumerate(self.convs):
            if self.gnn_type == "GINE":
                if i == 0:
                    h = conv(h_list[i], edge_index,edge_attr_in_channels)
                else:
                    h = conv(h_list[i], edge_index,edge_attr_hidden_channels)
            elif self.gnn_type == "GraphConv":
                h = conv(h_list[i], edge_index,edge_attr_change_one)
            else:   
                h = conv(h_list[i], edge_index)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gnn_type', help = "which gnn", action = 'store', default = 'GCN')
    ...
    parser.add_argument('--use_overall', help = "whether to use overall attr: 1 or 0", action = 'store', default = 0)
    ...
    parser.add_argument('--seed', help = "set seed", action = 'store', default = 1)
    ...
    parser.add_argument('--JK', help = "Jumping Knowledge :last ,sum,max,lstm", action = 'store', default = "last")
    ...
    conv_type = gnn_type # LASEConv / GraphConv / LEConv / etc
    num_fc_layers = 2
    ...
    dataset_dir = '{a}/dataset/sample'.format(a = PROJECT_ROOT)
    run_dir = '{a}/{z}_relation<{h}>_{b}_{c}_{d}_{e}_{f}_overall<{i}>_edge_feature<{j}>_nodes_feature<{k}>_onevone<{g}>_train_set<{l}>_aggregate<{m}>_pooling<{n}>_loss<{o}>_JK<{p}>'.format(a = model_dir, b = num_conv_layers, c = hidden_channels, d = batch_size, e = lr,f = drop_out,h = relations,i = overall_dim_large,j = edge_feature,k = node_feature,g =onevone, l = train_dataset,m = aggr_type , n = pool_type,o = loss_func,p = simple_JK,z = gnn_type)
    if use_overall:
    ...
    if args.train_dataset == "multi":
        ...
        dataset_list = generate_dataset(train_dataset_name,print_info = False)
        ...
        if test_dataset_name not in dataset_name_list:
            ...
            dataset_name_list.remove(test_dataset_name) 
        dataset_list = generate_dataset(dataset_name_list,print_info = False)  
    ...
    test_set = generate_dataset([test_dataset_name], print_info = False)
    # torch.save(train_set,"/home/yzk/LASE/1.7/lase_attr_overall/data/train_set.pt")
    # torch.save(val_set,"/home/yzk/LASE/1.7/lase_attr_overall/data/val_set.pt")
    # torch.save(test_set,"/home/yzk/LASE/1.7/lase_attr_overall/data/test_set.pt")
    # train_set = torch.load("/home/yzk/LASE/1.7/lase_attr_overall/data/train_set.pt")
    # val_set = torch.load("/home/yzk/LASE/1.7/lase_attr_overall/data/val_set.pt")
    # test_set = torch.load("/home/yzk/LASE/1.7/lase_attr_overall/data/test_set.pt")
    ...
    test_set = lase_direction_enhance(test_set, enhance = False,edge_attr_set= edge_feature,nodes_attr_set=node_feature,onevone=onevone,relation=relations,test = True)
    ...
    if os.path.exists('{}/best_model.pt'.format(run_dir)):
        ...
        model = GraphConvNet(in_channels = train_loader.dataset[0].num_node_features, hidden_channels = hidden_channels, 
            num_layers = num_conv_layers, dim = edge_dim, use_overall = use_overall, overall_dim = overall_dim,drop_out = drop_out,pool_aggr = pool_type,overall_dim_large=overall_dim_large,relations= relations,aggregate=aggr_type,simple_JK=simple_JK,gnn_type=gnn_type).to(device)
