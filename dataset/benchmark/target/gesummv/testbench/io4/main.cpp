...
extern "C" void gesummv(DATA_TYPE alpha, DATA_TYPE beta, DATA_TYPE A_0[N*N/4], DATA_TYPE A_1[N*N/4], DATA_TYPE A_2[N*N/4], DATA_TYPE A_3[N*N/4], 
	DATA_TYPE B_0[N*N/4], DATA_TYPE B_1[N*N/4], DATA_TYPE B_2[N*N/4], DATA_TYPE B_3[N*N/4], DATA_TYPE x[N], DATA_TYPE y_out[N]);
...
int main(int argc, char* argv[])
{
    ...
    for(inv = 0; inv < INV_NUM; inv++){
		...
		for(i = 0; i < N; i++){
		    for(j = 0; j < N; j++){
			    if(fscanf(fptr1, "%f", &read_din) > 0){
					...
					if(j%4 == 0) A_0[j_A_0++] = read_din;
					else if(j%4 == 1) A_1[j_A_1++] = read_din;
					else if(j%4 == 2) A_2[j_A_2++] = read_din;
					else A_3[j_A_3++] = read_din;
				}
		...
		for(i = 0; i < N; i++){
		    for(j = 0; j < N; j++){
			    if(fscanf(fptr2, "%f", &read_din) > 0){
					...
					if(j%4 == 0) B_0[j_B_0++] = read_din;
					else if(j%4 == 1) B_1[j_B_1++] = read_din;
					else if(j%4 == 2) B_2[j_B_2++] = read_din;
					else B_3[j_B_3++] = read_din;
				}
		...
		//==============================
		gesummv(alpha, beta, A_0, A_1, A_2, A_3, B_0, B_1, B_2, B_3, x, y_out);
